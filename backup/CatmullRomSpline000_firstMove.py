...
class CatmullRomSpline:
    ...
    def setControlPoint(self, index, coordinates):
        ...
        self.controlPoints[index] = coordinates

    def getXYCoordinatList(self):
        xList = []; yList = []
        for p in self.controlPoints.values():
            xList.append(p[0])
            yList.append(p[1])  
        return (xList, yList)
    ...
    def draw(self, figure):
        ...
        self.drawLinesBetweenControlPoints(figure)
        figure.gca().plot()
    ...
    def drawControlPoints(self, figure, handleRadius = 0.25):
        for p in self.controlPoints.values():
            ...
            self.artistToControlPoint[circle]=p #used for picking events
            
    def drawLinesBetweenControlPoints(self, figure):
    ...
    def selectControlPoint(self, artist):
        if(artist in self.artistToControlPoint):
            controlPoint = self.artistToControlPoint[artist]
            print(controlPoint)
            return(controlPoint)
...
def on_key_press(event):    
    ...
    if event.key == 'escape':    
        ...
        if movingElement is not None:
            movingElement[1] = movingElement[1] + 0.1
...
def on_pick(event):
    global movingElement
    movingElement = spline.selectControlPoint(event.artist)
    justSelected=True
...
def motion_notify_event(event):
    ...
    global figure
    if movingElement is not None:
        movingElement[0] = event.xdata
        movingElement[1] = event.ydata
    figure.clf()
    spline.draw(figure)
    figure.canvas.draw()
    justSelected = False
